...
ControlUINode::~ControlUINode()
{
    LOG_PRINT(1, "[ ControlUINode] Destroyed ControlUINode.\n");
}
...
ControlUINode::navDataCb(const ardrone_autonomy::Navdata navPtr)
{
    ...
    {
        LOG_MSG << "[ navDataCb] Current battery percentage: " << navPtr.batteryPercent << "\n";
        PRINT_LOG_MESSAGE(1);
        sendLand();
    }
...
{
    DEBUG_PRINT(10, "Receiving key point data.\n");
...
ControlUINode::write3DPointsToCSV(vector< vector<double> > &_3d_points, string filename)
{
    ...
    // Filename in which the points are to be written
    ...
    // Writing the points with width 6 (only 6 digits including significant and 
    // decimal digits would be written to file) to file
...
ControlUINode::fitPlane3d (vector<int> ccPoints, vector<vector<int> > pointsClicked)
{
    ...
    for(unsigned int i=0; i<_2d_points.size(); i++)
    {
        ...
        {
            DEBUG_MSG << _3d_points[i][0] << " " << _3d_points[i][1] << " " << _3d_points[i][2] << "\n";
            PRINT_DEBUG_MESSAGE(10);
            _in_points.push_back(_3d_points[i]);
        }
    }
    pthread_mutex_unlock(&keyPoint_CS);
    DEBUG_MSG << "Number of keypoints inside: " << _in_points.size() << ".\n";
    DEBUG_MSG << "Total number of keypoints: " << _3d_points.size() << ".\n";
    PRINT_DEBUG_MESSAGE(10);
                              ...
                              const vector< vector<Point3f> > &continuousBoundingBoxPoints)
{
    ...
    LOG_MSG << "[ moveQuadcopter] There are " << numberOfPlanes << " planes requested by user.\n";
    PRINT_LOG_MESSAGE(1);
    int i, j;
...
ControlUINode::printGrid(const pGrid &g, const vector<Point3f> &uvAxes, const vector<float> &plane)
{
    ...
    DEBUG_PRINT(1, "[ printGrid] UV Grid.\n");
...
{
    // Checking if 3D distance between two points is equal
...
void
ControlUINode::getPTargetPoints(const pGrid &g, const vector<float> & plane, int plane_no,
...
ControlUINode::adjustForNextCapture()
{
    ...
    if((_is_plane_covered) && (_node_completed_number_of_planes != _node_number_of_planes))
    {
...
ControlUINode::alignQuadcopterToNextPlane()
{
    ...
    cout << "[ DEBUG] [alignQuadcopterToNextPlane] Completed\n";
    return ;
}
